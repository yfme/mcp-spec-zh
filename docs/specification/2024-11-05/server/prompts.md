...
支持提示的服务器**必须**在[初始化]({{< ref "/specification/2024-11-05/basic/lifecycle#initialization" >}})期间声明 `prompts` 能力：
...
要检索可用的提示，客户端发送 `prompts/list` 请求。此操作支持[分页]({{< ref "/specification/2024-11-05/server/utilities/pagination" >}})。
...
要检索特定提示，客户端发送 `prompts/get` 请求。参数可以通过[完成 API]({{< ref "/specification/2024-11-05/server/utilities/completion" >}})自动完成。
